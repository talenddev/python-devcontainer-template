...
1. Run security review (global gate) → @agent-python-security-reviewer
2. Run docs (global gate) → @agent-python-docs-writer
...
1. **`docs/architecture-brief.md`** — primary source (from `python-architect`). If missing → request from `@agent-python-architect` before proceeding.
